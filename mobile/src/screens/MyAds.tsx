...
import {Text, VStack, HStack, Heading, Button, Select, useTheme, useToast, Center, FlatList} from 'native-base'
    ...
    useCallback(() => {
      const loadData = async () => {
        try {
          ...
          const title = isAppError ? error.message : 'Não foi possível receber seus anúncios. Tente Novamente!'
    ...
    <>
      {isLoading ? (
        ...
        <>
          ...
          <VStack mt={5} pt={5}>
            <HStack alignItems="center" mt={5} width="full" position="relative" justifyContent="center" px={5}>
              <Heading color="gray.200" fontSize={22} fontFamily="heading">
                Meus Anúncios
              </Heading>

              <Button variant="secondary" position="absolute" right={0} pr={5} onPress={handleGoCreateAd}>
                <Plus />
              </Button>
            </HStack>

            <HStack w="full" justifyContent="space-between" my={7} alignItems="center" px={5}>
