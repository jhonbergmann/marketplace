  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const title = isAppError ? error.message : 'Não foi possível receber os dados do anúncio. Tente Novamente!'
            ...
            <VStack flex={1}>
              ...
              <VStack px={5}>
                ...
                <Heading color="gray.300" fontSize={14} my={5}>
                  Aceita troca? <Text fontWeight="normal">{product.accept_trade ? 'Sim' : 'Não'}</Text>
                </Heading>
                ...
                {GeneratePaymentMethods(
                  product.payment_methods.map((payment_method) => payment_method.key),
                  colors.gray[300],
                )}
              </VStack>
            </VStack>
          </ScrollView>
          <HStack w="full" p={5} bg="white" alignItems="center" justifyContent="space-between">
            <Text color="blue.light" fontFamily="heading">
              R${' '}
            ...
            <Button title="Entrar em contato" icon={<WhatsappLogo color="white" weight="fill" size={20} />} alignItems="center" justifyContent="center" w="50%" />
          </HStack>
        </>
      )}
    </>
