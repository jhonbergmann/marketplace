...
import {HStack, VStack, Image, Heading, Text, Input, Modal, Button as NativeButton, Checkbox, Switch, Radio, useToast, FlatList, Center} from 'native-base'
import {Platform, TouchableOpacity} from 'react-native'
import {Controller, useForm} from 'react-hook-form'
import {useNavigation, useFocusEffect} from '@react-navigation/native'
import {Plus, BookmarkSimple, ArrowRight, MagnifyingGlass, Sliders} from 'phosphor-react-native'
...
export const Home = () => {
  ...
  const handleApplyFilters = async ({search}: FormDataProps) => {
    ...
    try {
      ...
      const productsData = await api.get(`/products/?is_new=${isNew}&accept_trade=${acceptTrade}${paymentMethodsQuery}${search.length > 0 && `&query=${search}`}`)
  ...
  return (
    <>
      {isLoading ? (
        ...
        <VStack flex={1} p={5}>
          <HStack w="full" mt={12} alignItems="center">
            ...
            <VStack ml="3">
              ...
              <Heading color="gray.200" fontSize={18}>
                {user.name.trim().split(' ')[0][0].toUpperCase() + user.name.trim().split(' ')[0].slice(1)}
              </Heading>
            </VStack>

            <Button title="Criar anúncio" icon={<Plus color="white" />} variant="default" width="40" h="12" position="absolute" right={0} onPress={handleCreateAd} />
          </HStack>
          ...
          <TouchableOpacity onPress={handleSeeMyAds}>
            <HStack alignItems="center" w="full" bg="gray.500" h="20" p="5" borderRadius="8" mt="2">
          ...
          <HStack w="full" alignItems="center" mt="2" bg="white" borderRadius={8} pr="1" mb="5">
            <Controller
              control={control}
              name="search"
              ...
              render={({field: {onChange, value}}) => (
                <>
                  ...
                  {errors.search?.message && (
                    <Text position="absolute" top="-20" right="4" color="red.light">
                      {errors.search?.message}
                    </Text>
                  )}
                </>
            ...
            <NativeButton onPress={handleSubmit(handleApplyFilters)} bg="transparent">
              <MagnifyingGlass weight="bold" color="#5F5B62" />
            </NativeButton>
            <NativeButton onPress={() => setShowFiltersModal(true)} bg="transparent">
              ...
              ListEmptyComponent={() => (
                <Center>
                  {isNew && !acceptTrade && paymentMethods.length === 5 ? (
                    ...
                    <Text color="gray.300" textAlign="center">
                      Parece que nenhum produto com esses filtros foi anunciado acima!
                    </Text>
                  )}
                </Center>
              )}
      ...
      <Modal isOpen={showFiltersModal} onClose={() => setShowFiltersModal(false)} size="xl">
        <Modal.Content marginBottom="5" marginTop="auto">
          ...
          <Modal.Body alignItems="flex-start" mb="5">
            ...
            <Switch onToggle={(value) => setAcceptTrade(value)} value={acceptTrade} size={Platform.OS === 'android' ? 'lg' : 'md'} m={0} />
          ...
          <Modal.Footer>
            <HStack w="full" py={2} px={5} bg="white" alignItems="center" justifyContent="space-between">
              <Button variant="secondary" title="Resetar Filtros" alignItems="center" justifyContent="center" w="47%" h={12} onPress={handleResetFilters} />
              <Button title="Aplicar Filtros" alignItems="center" justifyContent="center" w="47%" h={12} onPress={handleSubmit(handleApplyFilters)} />
            </HStack>
          </Modal.Footer>
        </Modal.Content>
      </Modal>
