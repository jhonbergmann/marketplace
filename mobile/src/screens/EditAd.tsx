...
import {ScrollView, VStack, Heading, Text, HStack, Image, Button as NativeButton, useTheme, Radio, Checkbox, Switch, useToast} from 'native-base'
...
const editAdSchema = yup.object({
  title: yup.string().required('Informe um título.').min(6, 'O título deve ter no mínimo 6 letras.'),
  description: yup.string().required('Informe uma descrição.').min(20, 'A descrição deve ser detalhada!'),
...
export const EditAd = () => {
  ...
  const {title, description, price, images: preImages, paymentMethods: prePaymentMethods, isNew: preIsNew, acceptTrade: preAcceptTrade, id} = route.params as RouteParams
  ...
  const handleAdPhotoSelect = async () => {
    try {
      ...
      const title = isAppError ? error.message : 'Não foi possível selecionar a imagem. Tente novamente!'
  ...
  return (
    <>
      <ScrollView contentContainerStyle={{flexGrow: 1}} showsVerticalScrollIndicator={false}>
        ...
        <VStack p={5} flex={1} alignItems="flex-start">
          ...
          <Controller
            ...
            render={({field: {onChange, value}}) => <Input placeholder="Título" onChangeText={onChange} value={value} errorMessage={errors.title?.message} />}
          />
          ...
          <Controller
            ...
            render={({field: {onChange, value}}) => <Input placeholder="0,00" h="14" mb={0} onChangeText={onChange} value={value} errorMessage={errors.price?.message} />}
          />
      ...
      <HStack w="full" py={2} px={5} bg="white" alignItems="center" justifyContent="space-between">
        <Button variant="secondary" title="Cancelar" alignItems="center" justifyContent="center" w="47%" h={12} onPress={handleGoBack} />
        <Button title="Avançar" alignItems="center" justifyContent="center" w="47%" h={12} onPress={handleSubmit(handleGoPreview)} />
      </HStack>
    </>
  )
}
