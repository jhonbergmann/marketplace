...
export const AdCard = ({title, price, used, active = true, image, profileImage, showProfile = false, id, ...rest}: Props) => {
    ...
    <TouchableOpacity onPress={handleGoMyAd}>
      <VStack position="relative" {...rest}>
        ...
        <Box position="relative" alignItems="center" justifyContent="center">
          {!active && (
            <Heading textTransform="uppercase" color="white" fontSize={10} position="absolute" zIndex={100} bg="gray.300" p={1} borderRadius={10}>
              Anúncio Desativado
            </Heading>
          )}
          ...
          <Image h="24" w="lg" source={{uri: image}} alt={title} resizeMode="cover" borderRadius={10} blurRadius={active ? 0 : 10} borderWidth={1} borderColor="gray.500" />
        </Box>
